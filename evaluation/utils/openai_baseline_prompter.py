...
class OpenAIPrompter:
    def __init__(self, model):
        self.client = lmql.model_registry.get("gpt2-medium")
    ...
    async def generate(self, prompt, remove_stopping_phrases = True, truncate=None, **kwargs):
        input_ids = np.array([self.client.bos_token_id] + (await self.client.tokenize(prompt)), dtype=np.int64).reshape(-1)
        ...
        while len(s.item(0).input_ids) < max_length:
            ...
            for _ in range(max_length_step):
                try:
